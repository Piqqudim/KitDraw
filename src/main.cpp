...
#include "Renderer/ViewportScope.hpp"
...
int main()
{
    ...
    { 
        ...
        canvas.unbind();
        const glm::vec2 canvasCenter{canvas.width() * 0.5f, canvas.height() * 0.5f};
        camera.reset(canvasCenter);
        const float fitZoomX = (float)window.width() / (float)canvas.width();
        const float fitZoomY = (float)window.height() / (float)canvas.height();
        const float fitZoom = std::min(fitZoomX, fitZoomY) * 0.9f; 
                        ...
                        else if(e.button == GLFW_MOUSE_BUTTON_LEFT)
                        {
                            ...
                            if(painting){
                                glm::vec2 canvasPos = camera.screenToWorld(screenpos);
                                 canvas.bind();
                                 KitDraw::render::ViewportScope vp(gfx, 0, 0, canvas.width(), canvas.height(),window.width(), window.height());
